...
1. 最终数据大表（也就是各种方法在 4 个数据集、11 个指标上的得分）打印出来 
	1. 搞懂，shdf在一个数据集上的所有指标如何跑出来
	2. 然后，数据表就出来了，就可对比。
	   谁是全场总冠军？ 哪种方法在几乎所有几何指标（CD/EMD）上都压倒性获胜？
		谁在点对应（Uncertainty/SRE）上最强？ 是不是形变场流派（NDF/SHDF）虽然几何精度不是最高，但点对应和配准误差低得惊人？
		
		谁在极度稀疏的情况下摆烂了？ 当采样点数从 500K 降到 100K 时，哪个方法的 CD 飙升得最厉害？
		
		谁生成的网格全是流形错误？ 哪些方法虽然 CD 好看，但自相交率（Self-Intersection）高达 15%？
1. 
