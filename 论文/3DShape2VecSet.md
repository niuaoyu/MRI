...
这里VAE的核心一点，原本数据有自己的分布，但强行拉回到正态分布的同时还能保持原有信息的一个原因是，通过不同维度来看的，咋理解，比如岛屿 A 聚集了椅子的特征，岛屿 B 聚集了汽车的特征，岛屿 A 和岛屿 B 之间，隔着大片巨大的、没有任何数据落进去过的“物理真空死海”。VAE就是通过某个角度（超维平面）让这些数据符合正态分布的同时又不丢失太多的信息。实现的原理是通过损失loss=loss（标准状态分布）+loss（重建损失），平衡好这两个损失的关系，让编码器能拉到标准正态分布的时候，重建的效果也不错。
