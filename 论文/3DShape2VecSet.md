...
1. 为什么VAE自编码器过程当中要有KL这个模块？
	...
	2. 也可能会问，没法比就没法比，反正只要强迫模型学一个从标准正态分布 $\mathcal{N}(0, I)$ 到任意真实潜在分布 $p(z)$ 的映射轨迹就行了。这种思想是流匹配（Flow Matching）**和**流序生成模型（Flow-based Models）的底层逻辑，但如果在“扩散模型（Diffusion Model）”的框架下，不让标签（VAE 输出）变成标准正态分布，训练就会直接崩溃。
